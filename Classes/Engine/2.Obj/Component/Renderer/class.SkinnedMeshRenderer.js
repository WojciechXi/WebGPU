class SkinnedMeshRenderer extends Renderer {
    ...
    constructor(data = {}, parameters = {}) {
        super(data, {
            ...
            receiveShadows: { value: data._receiveShadows ?? data.receiveShadows ?? true, },
            castShadows: { value: data._castShadows ?? data.castShadows ?? true, },
            materials: { value: data._materials ?? data.materials ?? [null], },
            mesh: { value: data._mesh ?? data.mesh ?? null },
            skin: { value: data._skin ?? data.skin ?? null },
            bindings: { value: [] },
        });
    }
    ...
    OnDraw(renderPass, camera) {
        ...
        if (renderPass.name === 'shadowRenderPass') {
            ...
            for (let i = 0; i < this.mesh.subMeshCount; i++) {
                renderPass.DrawSkinnedMesh(this.mesh, i, this.transform.transformBuffer.buffer, this.jointsBindGroup);
            }
