class Editor {
    ...
    constructor() {
        const object = this;
        object.view = document.querySelector('#view');
        object.inspector = document.querySelector('#inspector');
        object.hierarchy = document.querySelector('#hierarchy');
    ...
    Select(items) {
        const object = this;
        object.selected = Array.isArray(items) ? items : items ? [items] : [];
        object.UpdateInspector();
    }

    async Init() {
        const object = this;
        object.device = await GPU.Request();
        ...
        Ajax.Get('/assets.php', function (response) {
            ...
            const engine = object.engine = new Engine(assets);
    ...
    UpdateHierarchy() {
        const object = this;
        object.hierarchy.innerHTML = null;
        for (const gameObject of object.engine.scene.gameObjects) {
            let div = document.createElement('div');
            object.hierarchy.appendChild(div);
            div.innerText = gameObject.name;
            div.addEventListener('click', function (event) {
                object.Select([gameObject]);
            });
        }
    }
    ...
    UpdateInspector() {
        const object = this;
        object.inspector.innerHTML = null;
        if (object.selected) {
            for (const item of object.selected) {
                let div = document.createElement('div');
                object.inspector.appendChild(div);
                div.innerText = item.name;

                let divComponents = document.createElement('div');
                object.inspector.appendChild(divComponents);
                if (item instanceof GameObject) {
                    for (let component of item.components) {
                        if (Editor.editors[component.GetType()]) {
                            let e = new (Editor.editors[component.GetType()])(component);
                            divComponents.appendChild(e.Render());
                        }
                    }
                }
                break;
