...
struct SSAOUniforms {
  radius       : f32,
  bias         : f32,
  viewProj     : mat4x4f,          // viewProjection matrix
  invViewProj  : mat4x4f,          // inverse viewProjection
...
fn fs(input: VSOut) -> FSOut {
  ...
  for (var i = 0u; i < 32u; i = i + 1u) {
    ...
    // rzut do clip-space i NDC
    let clipPos = ssaoUni.viewProj * vec4f(samplePos, 1.0);
    let ndc = clipPos.xyz / clipPos.w;
    let uv = ndc.xy * 0.5 + 0.5;

    // sprawdzanie granic tekstury
