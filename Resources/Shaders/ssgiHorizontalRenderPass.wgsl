...
fn fs(in: VSOut) -> @location(0) vec4<f32> {
    ...
    let texelSize = vec2<f32>(1.0 / texSize.x, 0.0); // Offset tylko w osi X
    ...
    let centerNormal = normalize(textureSampleLevel(normalTexture, samplerPoint, uv, 0).xyz);

    var totalColor = centerColor;
    var totalWeight = 1.0;

    // Wagi Gaussa dla promienia = 4 (kernel 9x9)
    let weights = array<f32, 4>(0.26, 0.16, 0.07, 0.01);

    for (var i = 1; i <= 4; i = i + 1) {
        let offset = vec2<f32>(f32(i)) * texelSize;

        // Próbka dodatnia i ujemna w osi X
        let uvP = uv + offset;
        let uvN = uv - offset;

        // --- Próbka P ---
        let depthP = linearizeDepth(textureSampleLevel(depthTexture, samplerPoint, uvP, 0));
        let normalP = normalize(textureSampleLevel(normalTexture, samplerPoint, uvP, 0).xyz);
        ...
        let normalN = normalize(textureSampleLevel(normalTexture, samplerPoint, uvN, 0).xyz);
